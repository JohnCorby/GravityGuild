...
public class Arena {
	...
	public boolean add(Entity entity) {
		...
		if (entity instanceof Player) {
			...
			if (state == State.RUNNING && !overridePlayers.contains(p))
    ...
    /*
    private Runnable getBlocks = new Runnable() {
        int x, z, y;
        ...
        public synchronized BukkitTask runTaskTimer(long delay, long period) {
            ...
            y = 0;
            z = region[1];
            return super.runTaskTimer(delay, period);
        }
	...
	*/
    ...
    private void getBlocks() {
        ...
        Vector pos1 = new Vector(region[0], 0, region[2]);
        Vector pos2 = new Vector(region[1], 255, region[3]);
        World world = BukkitUtil.getLocalWorld(WORLD);

        try {
            new Schematic(new CuboidRegion(world, pos1, pos2)).save(file, ClipboardFormat.SCHEMATIC);
        } catch (IOException e) {
    ...
    /*
    private Runnable setBlocks = new Runnable() {
        int i;
        ...
        public void run() {
            for (int t = 0; t < 100000; t++) {
                ...
                //blocks.get(i).getBlock().setType(Material.DIRT);
                blocks.get(i).update();
                i++;
            }
        }
    ...
    */
    ...
    private void schemStuff() {
	    try {
	        ...
	        error("Loading schematic for arena: " + getStackTrace(e));
        }
    }
    */
